...
结构四件事已落地（流式 / 程序图集 / AO / 描边裂纹）。程序图集只把彩色盒子变成「有点像 MC 的演示」；用户仍觉得不好看。

- 不要用本文短提示或 docsbot 长提示整仓重写。
- 不要接 Oasis / Marble / Meshy 网格（见 [调研](./2026-08-17-一句话生成我的世界3D调研.md)）。
- 下一刀仍是本文第 2 条，但升级为**手绘 16×16 像素图集**，不是再画一遍噪声。落点：[美术后续优化方案](./2026-08-17-美术后续优化方案.md)。