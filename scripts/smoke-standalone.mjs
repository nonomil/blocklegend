...
assert.match(html, /words\.js\?v=20260819-bl-survive1/);
assert.match(html, /speech-input\.js\?v=20260819-bl-scene1/);
assert.match(html, /game\.js\?v=20260819-bl-survive1/);
assert.match(html, /选一个玩法/);
...
assert.match(levels, /FIRST_WAVE_COUNT = 6/);
assert.match(words, /function speakQueue/);
assert.match(words, /WORLD_QUIZ_EVERY = 8/);
assert.match(game, /function maybeOpenWorldQuiz/);
assert.match(game, /function recordBridgeAnswer/);
assert.match(game, /function tryEat/);
assert.match(game, /function openCraftQuiz/);
assert.match(game, /function killAnimal/);
assert.doesNotMatch(game, /0\.25 \* dt/);
assert.match(words, /function dueHouseAwards/);
const tools = fs.readFileSync('data/tools.js', 'utf8');
assert.match(tools, /DEFAULT_HOTBAR = \['fist'/);
assert.match(fs.readFileSync('LICENSE', 'utf8'), /GNU AFFERO GENERAL PUBLIC LICENSE/);
